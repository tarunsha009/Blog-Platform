...
@patch('blog_platform.services.user_services.UserDBService.get_user_by_username')
@patch('blog_platform.services.user_services.UserDBService.create_user')
def test_register_user_success(mock_create_user, mock_get_user_by_username, user_data):
    ...
    assert response == {"message": "User registered successfully", "user_id": 1}
...
def test_login_user_success(mock_get_user_by_username, user_data, mocker, app):
    with app.app_context():
        ...
        mocker.patch('flask_jwt_extended.create_access_token', return_value='testtoken')

        token = UserService.login_user(user_data['username'], user_data['password'])
...
@patch('blog_platform.services.user_services.UserDBService.get_user_by_username')
def test_login_user_invalid_password(mock_get_user_by_username, user_data, mocker):
    # Mock user data and password check
